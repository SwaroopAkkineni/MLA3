...
def network(inputs):
    ...
    with slim.arg_scope([slim.conv2d, slim.fully_connected],
                      ...
                      weights_regularizer=slim.l2_regularizer(0.01)):#changed
        net = tf.reshape(inputs,[-1, 512,512,3])
        net = slim.conv2d(net, 32, [3,3], scope='conv1')   # weights, kernal size
        net = slim.max_pool2d(net, [4,4], scope = 'conv1') # how much it reducing
        net = slim.conv2d(net,64,[3,3], scope = 'conv2')
        net = slim.max_pool2d(net,[4,4], scope = 'pool2')
        net = slim.conv2d(net,64,[3,3], scope = 'conv3')# additions
